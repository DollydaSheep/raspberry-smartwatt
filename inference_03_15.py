...
def process_manual_command(label: str, action: str, matched_state_w: float):
    ...
    if action == "ON":
        ...
        print(f"[{ph_now_str()}] [CMD ON] {label} | state={matched_state_w:.1f}W")
        return True
        ...
        print(f"[{ph_now_str()}] [CMD OFF] {label}")
        return True
    ...
    try:
        ...
        if event:
            print(
                ...
                f"(state {event['matched_state']:.1f}W, diff {event['diff']:.1f}W, at {event['timestamp_iso']})"
            )
        else:
