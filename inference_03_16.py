import json
import pickle
import time
from collections import deque
from datetime import datetime, timezone
from zoneinfo import ZoneInfo

import paho.mqtt.client as mqtt
from supabase import create_client, Client

MODEL_PATH = "co_model_house_calibrate.pkl"

SUPABASE_URL = "https://yuvamvpfxhzuvvnvtqhj.supabase.co"
SUPABASE_SERVICE_ROLE_KEY = "YOUR_SERVICE_ROLE_KEY"

supabase: Client = create_client(SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY)

LIVE_ROW_ID = 1
PH_TZ = ZoneInfo("Asia/Manila")

# -----------------------------
# TUNABLE PARAMETERS
# -----------------------------
SMOOTHING_WINDOW = 3
EVENT_THRESHOLD_W = 25.0
EVENT_DEBOUNCE_SEC = 2.0
GLOBAL_TOLERANCE_FLOOR = 20.0

DEFAULT_RULES = {
    "min_on_sec": 5.0,
    "cooldown_sec": 3.0,
}

APPLIANCE_RULES = {
    "Fridge freezer": {"min_on_sec": 60.0, "cooldown_sec": 10.0},
    "Washer dryer": {"min_on_sec": 30.0, "cooldown_sec": 10.0},
    "Light": {"min_on_sec": 2.0, "cooldown_sec": 1.0},
    "HTPC": {"min_on_sec": 10.0, "cooldown_sec": 3.0},
}

# -----------------------------
# LOAD MODEL
# -----------------------------
with open(MODEL_PATH, "rb") as f:
    model = pickle.load(f)

labels = model["labels"]
states = model["states"]
tolerance = model.get("tolerance", [50.0] * len(labels))

print("SMART-WATT Event-Based NILM Started")

# -----------------------------
# NILM RUNTIME STATE
# -----------------------------
power_buffer = deque(maxlen=SMOOTHING_WINDOW)
last_smoothed_power = None
last_raw_power = None
last_event_time = 0.0

appliance_runtime = []
for idx, (label, appliance_states, tol) in enumerate(zip(labels, states, tolerance)):
    numeric_states = sorted(
        {
            float(s)
            for s in appliance_states
            if isinstance(s, (int, float)) or str(s).replace(".", "", 1).isdigit()
        }
    )
    nonzero_states = [s for s in numeric_states if s > 0]

    appliance_runtime.append({
        "id": idx,
        "label": label,
        "states": nonzero_states,
        "tolerance": max(float(tol), GLOBAL_TOLERANCE_FLOOR),
        "is_on": False,
        "matched_state": 0.0,
        "last_on_time": 0.0,
        "last_off_time": 0.0,
        "last_change_time": 0.0,
        "open_event_id": None,
        "event_start_ts": 0.0,
        "last_raw_on_delta": None,
        "last_smoothed_on_delta": None,
        "last_raw_off_delta": None,
        "last_smoothed_off_delta": None,
    })

# -----------------------------
# ENERGY + AGGREGATION STATE
# -----------------------------
last_raw_ts = None
total_energy_kwh = 0.0

current_second_epoch = None
current_second_bucket = {
    "sum_voltage": 0.0,
    "sum_current": 0.0,
    "sum_power": 0.0,
    "count": 0,
}

minute_second_logs = []
current_minute_key = None

latest_voltage = 0.0
latest_current = 0.0
latest_power = 0.0


def ph_now_iso():
    return datetime.now(PH_TZ).isoformat()


def ph_now_str():
    return datetime.now(PH_TZ).strftime("%Y-%m-%d %H:%M:%S %Z")


def ts_to_ph_iso(ts: float):
    return datetime.fromtimestamp(ts, tz=PH_TZ).isoformat()


def normalize_label(label: str) -> str:
    return str(label).strip().lower()


def reset_second_bucket():
    return {
        "sum_voltage": 0.0,
        "sum_current": 0.0,
        "sum_power": 0.0,
        "count": 0,
    }


def get_rules(label: str) -> dict:
    return APPLIANCE_RULES.get(label, DEFAULT_RULES)


def smooth_power(power: float) -> float:
    power_buffer.append(power)
    return sum(power_buffer) / len(power_buffer)


def current_detected_appliances():
    detected = []
    for app in appliance_runtime:
        if app["is_on"]:
            detected.append({
                "id": app["id"],
                "label": app["label"],
                "matched_state": float(app["matched_state"]),
            })
    return detected


def get_runtime_app_by_label(label: str):
    target = normalize_label(label)
    for app in appliance_runtime:
        if normalize_label(app["label"]) == target:
            return app
    return None


def get_default_state_for_app(app):
    if app["states"]:
        return float(app["states"][0])
    return 0.0


def insert_appliance_event_on(app, source="manual_app"):
    payload = {
        "appliance_label": app["label"],
        "started_at": ts_to_ph_iso(app["event_start_ts"]),
        "matched_state_w": float(app["matched_state"]),
        "status": "open",
        "created_at": ph_now_iso(),
        "updated_at": ph_now_iso(),
        "event_source": source,
        "raw_delta_w": app["last_raw_on_delta"],
        "smoothed_delta_w": app["last_smoothed_on_delta"],
    }

    res = supabase.table("appliance_events").insert(payload).execute()

    try:
        inserted = res.data[0]
        return inserted["id"]
    except Exception:
        return None


def close_appliance_event_off(app, source="manual_app"):
    if not app["open_event_id"]:
        return

    started_at_ts = app["event_start_ts"]
    ended_at_ts = time.time()
    duration_sec = max(0.0, ended_at_ts - started_at_ts)

    matched_state_w = float(app["matched_state"])
    estimated_energy_kwh = (matched_state_w * duration_sec) / 3600000.0

    payload = {
        "ended_at": ts_to_ph_iso(ended_at_ts),
        "duration_sec": float(duration_sec),
        "estimated_energy_kwh": float(estimated_energy_kwh),
        "status": "closed",
        "updated_at": ph_now_iso(),
        "event_source": source,
        "raw_off_delta_w": app["last_raw_off_delta"],
        "smoothed_off_delta_w": app["last_smoothed_off_delta"],
    }

    supabase.table("appliance_events").update(payload).eq("id", app["open_event_id"]).execute()

    print(
        f"[{ph_now_str()}] [EVENT CLOSE] {app['label']} | "
        f"duration={duration_sec:.1f}s | energy={estimated_energy_kwh:.6f} kWh | "
        f"raw_off={app['last_raw_off_delta']}W | smooth_off={app['last_smoothed_off_delta']}W"
    )

    app["open_event_id"] = None
    app["event_start_ts"] = 0.0
    app["last_raw_off_delta"] = None
    app["last_smoothed_off_delta"] = None


def find_best_on_candidate(delta_p: float, now_ts: float):
    best = None

    for app in appliance_runtime:
        if app["is_on"]:
            continue

        rules = get_rules(app["label"])
        if now_ts - app["last_change_time"] < rules["cooldown_sec"]:
            continue

        for state in app["states"]:
            diff = abs(delta_p - state)
            if diff <= app["tolerance"]:
                candidate = {
                    "app": app,
                    "target_state": state,
                    "diff": diff,
                }
                if best is None or candidate["diff"] < best["diff"]:
                    best = candidate

    return best


def find_best_off_candidate(delta_p: float, now_ts: float):
    best = None
    abs_delta = abs(delta_p)

    for app in appliance_runtime:
        if not app["is_on"]:
            continue

        rules = get_rules(app["label"])
        if now_ts - app["last_change_time"] < rules["cooldown_sec"]:
            continue

        if now_ts - app["last_on_time"] < rules["min_on_sec"]:
            continue

        expected_drop = app["matched_state"]
        diff = abs(abs_delta - expected_drop)

        if diff <= app["tolerance"]:
            candidate = {
                "app": app,
                "target_state": 0.0,
                "diff": diff,
            }
            if best is None or candidate["diff"] < best["diff"]:
                best = candidate

    return best


def apply_event(candidate: dict, raw_delta_p: float, smoothed_delta_p: float, now_ts: float):
    app = candidate["app"]

    if raw_delta_p > 0:
        app["is_on"] = True
        app["matched_state"] = float(candidate["target_state"])
        app["last_on_time"] = now_ts
        app["last_change_time"] = now_ts
        app["event_start_ts"] = now_ts
        app["last_raw_on_delta"] = float(raw_delta_p)
        app["last_smoothed_on_delta"] = float(smoothed_delta_p)

        event = {
            "event": "ON",
            "id": app["id"],
            "label": app["label"],
            "matched_state": float(app["matched_state"]),
            "raw_delta": float(raw_delta_p),
            "smoothed_delta": float(smoothed_delta_p),
            "diff": float(candidate["diff"]),
            "timestamp": now_ts,
            "timestamp_iso": ts_to_ph_iso(now_ts),
        }
    else:
        previous_state = app["matched_state"]
        app["is_on"] = False
        app["last_off_time"] = now_ts
        app["last_change_time"] = now_ts
        app["last_raw_off_delta"] = float(raw_delta_p)
        app["last_smoothed_off_delta"] = float(smoothed_delta_p)

        event = {
            "event": "OFF",
            "id": app["id"],
            "label": app["label"],
            "matched_state": float(previous_state),
            "raw_delta": float(raw_delta_p),
            "smoothed_delta": float(smoothed_delta_p),
            "diff": float(candidate["diff"]),
            "timestamp": now_ts,
            "timestamp_iso": ts_to_ph_iso(now_ts),
        }
        app["matched_state"] = 0.0

    return event


def process_event(raw_delta_p: float, smoothed_delta_p: float, now_ts: float):
    global last_event_time

    if abs(raw_delta_p) < EVENT_THRESHOLD_W:
        return None

    if now_ts - last_event_time < EVENT_DEBOUNCE_SEC:
        return None

    if raw_delta_p > 0:
        candidate = find_best_on_candidate(raw_delta_p, now_ts)
    else:
        candidate = find_best_off_candidate(raw_delta_p, now_ts)

    if candidate is None:
        return None

    last_event_time = now_ts
    return apply_event(candidate, raw_delta_p, smoothed_delta_p, now_ts)


def update_live_reading(voltage, current, power, smoothed_power, delta_p, appliances, last_event, total_kwh):
    payload = {
        "id": LIVE_ROW_ID,
        "voltage": float(voltage),
        "current": float(current),
        "power": float(power),
        "detected_appliances": appliances,
        "updated_at": ph_now_iso(),
    }

    return supabase.table("energy_readings").upsert(payload).execute()


def flush_minute_aggregate(minute_key: str):
    global minute_second_logs

    if not minute_second_logs:
        return

    n = len(minute_second_logs)

    avg_voltage = sum(x["voltage"] for x in minute_second_logs) / n
    avg_current = sum(x["current"] for x in minute_second_logs) / n
    avg_power = sum(x["power_w"] for x in minute_second_logs) / n
    minute_energy_kwh = sum(x["energy_kwh"] for x in minute_second_logs)

    payload = {
        "power_w": float(avg_power),
        "energy_kwh": float(minute_energy_kwh),
        "voltage": float(avg_voltage),
        "current": float(avg_current),
        "recorded_at": f"{minute_key}:00+08:00",
    }

    try:
        supabase.table("energy_readings_aggregate").insert(payload).execute()
        print(
            f"[{ph_now_str()}] [AGG INSERT] {payload['recorded_at']} | "
            f"Pavg={avg_power:.2f}W | Vavg={avg_voltage:.2f}V | "
            f"Iavg={avg_current:.3f}A | E={minute_energy_kwh:.6f} kWh"
        )
    except Exception as e:
        print(f"[{ph_now_str()}] Error inserting minute aggregate:", e)

    minute_second_logs = []


def finalize_second_log(second_epoch: int):
    global current_second_bucket, minute_second_logs, current_minute_key

    if current_second_bucket["count"] == 0:
        return

    avg_voltage = current_second_bucket["sum_voltage"] / current_second_bucket["count"]
    avg_current = current_second_bucket["sum_current"] / current_second_bucket["count"]
    avg_power = current_second_bucket["sum_power"] / current_second_bucket["count"]

    second_energy_kwh = avg_power / 3600000.0
    second_dt = datetime.fromtimestamp(second_epoch, tz=PH_TZ)
    minute_key = second_dt.strftime("%Y-%m-%dT%H:%M")

    if current_minute_key is None:
        current_minute_key = minute_key
    elif minute_key != current_minute_key:
        flush_minute_aggregate(current_minute_key)
        current_minute_key = minute_key

    minute_second_logs.append({
        "voltage": float(avg_voltage),
        "current": float(avg_current),
        "power_w": float(avg_power),
        "energy_kwh": float(second_energy_kwh),
        "recorded_at": second_dt.isoformat(),
    })

    current_second_bucket = reset_second_bucket()


def add_raw_sample_to_second_bucket(ts_epoch: float, voltage: float, current: float, power: float):
    global current_second_epoch, current_second_bucket

    sec = int(ts_epoch)

    if current_second_epoch is None:
        current_second_epoch = sec

    if sec != current_second_epoch:
        finalize_second_log(current_second_epoch)
        current_second_epoch = sec

    current_second_bucket["sum_voltage"] += voltage
    current_second_bucket["sum_current"] += current
    current_second_bucket["sum_power"] += power
    current_second_bucket["count"] += 1


def update_total_energy(power_w: float, now_ts: float):
    global last_raw_ts, total_energy_kwh

    if last_raw_ts is None:
        last_raw_ts = now_ts
        return total_energy_kwh

    dt_sec = now_ts - last_raw_ts
    if dt_sec < 0:
        dt_sec = 0

    total_energy_kwh += (power_w * dt_sec) / 3600000.0
    last_raw_ts = now_ts
    return total_energy_kwh


def process_manual_command(label: str, action: str, matched_state_w: float):
    app = get_runtime_app_by_label(label)

    if app is None:
        print(f"[{ph_now_str()}] [CMD] Appliance not found in model: {label}")
        return False

    now_ts = time.time()

    if action == "ON":
        if app["is_on"]:
            print(f"[{ph_now_str()}] [CMD] {label} already ON")
            return False

        if matched_state_w <= 0:
            matched_state_w = get_default_state_for_app(app)

        if matched_state_w <= 0:
            print(f"[{ph_now_str()}] [CMD] No valid matched_state_w for {label}")
            return False

        app["is_on"] = True
        app["matched_state"] = float(matched_state_w)
        app["last_on_time"] = now_ts
        app["last_change_time"] = now_ts
        app["event_start_ts"] = now_ts

        # manual ON spike should follow predicted appliance wattage
        app["last_raw_on_delta"] = float(matched_state_w)
        app["last_smoothed_on_delta"] = float(matched_state_w)

        open_id = insert_appliance_event_on(app, source="manual_app")
        app["open_event_id"] = open_id

        print(
            f"[{ph_now_str()}] [CMD ON] {label} | "
            f"state={matched_state_w:.1f}W | "
            f"raw_on_delta={app['last_raw_on_delta']:+.1f}W | "
            f"smoothed_on_delta={app['last_smoothed_on_delta']:+.1f}W"
        )
        return True

    elif action == "OFF":
        if not app["is_on"]:
            print(f"[{ph_now_str()}] [CMD] {label} already OFF")
            return False

        previous_state = float(app["matched_state"])

        # manual OFF drop should follow predicted appliance wattage
        app["last_raw_off_delta"] = -previous_state
        app["last_smoothed_off_delta"] = -previous_state

        close_appliance_event_off(app, source="manual_app")

        app["is_on"] = False
        app["matched_state"] = 0.0
        app["last_off_time"] = now_ts
        app["last_change_time"] = now_ts

        print(
            f"[{ph_now_str()}] [CMD OFF] {label} | "
            f"raw_off_delta={app['last_raw_off_delta']:+.1f}W | "
            f"smoothed_off_delta={app['last_smoothed_off_delta']:+.1f}W"
        )
        return True

    else:
        print(f"[{ph_now_str()}] [CMD] Unknown action: {action}")
        return False


def poll_appliance_commands():
    try:
        res = (
            supabase.table("appliance_commands")
            .select("*")
            .eq("status", "pending")
            .order("created_at")
            .execute()
        )

        rows = res.data or []

        for row in rows:
            cmd_id = row["id"]
            label = row["appliance_label"]
            action = str(row["action"]).upper()
            matched_state_w = float(row.get("matched_state_w") or 0)

            print(f"[{ph_now_str()}] [CMD] id={cmd_id} | label={label} | action={action}")

            process_manual_command(label, action, matched_state_w)

            supabase.table("appliance_commands").update({
                "status": "processed",
                "processed_at": ph_now_iso(),
            }).eq("id", cmd_id).execute()

    except Exception as e:
        print(f"[{ph_now_str()}] [CMD POLL ERROR]", e)


def on_message(client, userdata, msg):
    global last_smoothed_power, last_raw_power, latest_voltage, latest_current, latest_power

    try:
        payload_str = msg.payload.decode("utf-8")
        data = json.loads(payload_str)

        voltage = float(data.get("voltage", 0))
        current = float(data.get("current", 0))
        power = float(data.get("power", 0))

        latest_voltage = voltage
        latest_current = current
        latest_power = power

        now_ts = time.time()

        total_kwh = update_total_energy(power, now_ts)
        add_raw_sample_to_second_bucket(now_ts, voltage, current, power)

        smoothed = smooth_power(power)

        if last_raw_power is None:
            last_raw_power = power
            last_smoothed_power = smoothed

            delta_p = 0.0
            detected = current_detected_appliances()
            update_live_reading(voltage, current, power, smoothed, delta_p, detected, None, total_kwh)

            print(
                f"[{ph_now_str()}] Init | "
                f"V={voltage:.1f}V I={current:.2f}A "
                f"P={power:.1f}W Smooth={smoothed:.1f}W Energy={total_kwh:.6f} kWh"
            )
            return

        raw_delta_p = power - last_raw_power
        smoothed_delta_p = smoothed - last_smoothed_power

        event = process_event(raw_delta_p, smoothed_delta_p, now_ts)

        if event:
            app = appliance_runtime[event["id"]]

            if event["event"] == "ON":
                app["event_start_ts"] = event["timestamp"]
                open_id = insert_appliance_event_on(app, source="nilm")
                app["open_event_id"] = open_id

            elif event["event"] == "OFF":
                app["matched_state"] = float(event["matched_state"])
                close_appliance_event_off(app, source="nilm")
                app["matched_state"] = 0.0

        detected = current_detected_appliances()

        print(
            f"[{ph_now_str()}] "
            f"V={voltage:.1f}V | I={current:.2f}A | "
            f"P={power:.1f}W | Smooth={smoothed:.1f}W | "
            f"dP(raw)={raw_delta_p:+.1f}W | dP(smooth)={smoothed_delta_p:+.1f}W | "
            f"Energy={total_kwh:.6f} kWh"
        )

        if event:
            print(
                f"[{ph_now_str()}] EVENT: {event['event']} -> {event['label']} "
                f"(state {event['matched_state']:.1f}W, "
                f"raw {event['raw_delta']:+.1f}W, "
                f"smooth {event['smoothed_delta']:+.1f}W, "
                f"diff {event['diff']:.1f}W, at {event['timestamp_iso']})"
            )
        else:
            print(f"[{ph_now_str()}] EVENT: none")

        if detected:
            print(f"[{ph_now_str()}] Detected appliances:")
            for app in detected:
                print(f"  - {app['label']} (state {app['matched_state']:.1f}W)")
        else:
            print(f"[{ph_now_str()}] Detected appliances: []")

        update_live_reading(voltage, current, power, smoothed, raw_delta_p, detected, event, total_kwh)

        last_raw_power = power
        last_smoothed_power = smoothed

    except json.JSONDecodeError:
        print(f"[{ph_now_str()}] Error: received invalid JSON payload")
    except Exception as e:
        print(f"[{ph_now_str()}] Error:", e)


client = mqtt.Client()
client.on_message = on_message

client.connect("localhost", 1883)
client.subscribe("smartwatt/readings")
client.loop_start()

try:
    while True:
        poll_appliance_commands()
        time.sleep(1)
except KeyboardInterrupt:
    print(f"[{ph_now_str()}] Stopping script...")
finally:
    client.loop_stop()
    client.disconnect()